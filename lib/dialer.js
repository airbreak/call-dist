...
var DialerEnv = (function() {
    ...
    env.init = function(callback) {
        ...
        if (ua.android) {
            ...
            if (jsHandler) {
                ...
                BaseParams.remain = params.remainMinutes;
                ...
                if (BaseParams.phone) {
                    BaseParams.status = env.STATUS.LOGGED;
                }
            } else {
    ...
    /**
     * @description   转换分享的路径信息
     * 某些场景可能需要转换html元素的css类名，所以在执行分享操作后需要记录readable的信息<br/>
     * [调用示例]
     <pre>
     var recName = _dialerEnv.convRecName("icon wechat");
     </pre>
     [源码]
     * @param {String}   org_approach  原始路径
     * @return {Objcet}  返回转换后的分享路径信息
     **/
    env.convRecName = function(org_approach) {
        var approach = "";
        var record_name = "";
        var app_name = "";
    ...
    env.doShare = function(approach, params, directly, callback) {
        ...
        if (callback) {
            registerAsWindowFunction.call(CallbackFunction, "shareSucceed", function(share_from, org_approach, status) {
                var processedName = env.convRecName(org_approach);
        ...
        if (params.img_url) {
            (params.image_url = params.img_url);
        }
        var paramsJson = {
            "approach": approach,
            "params": params,
    ...
    env.pushWeb = function(url, title, file_name, callback) {
        ...
        if (ua.android && jsHandler) {
            ...
            (jsHandler) &&
            (ret = (jsHandler.pushWeb(JSON.stringify(json))));
     ...
     * <br/>调用示例(1)：[Android] without extraData
     ...
     [源码]
     * @param {Json} clsName     活动名称(包名/Conrtoller名称)
        ...
        if (ua.android && jsHandler) {
            ...
            if (extraData) {
                ...
                if (intentData || jsonExtraData || startService) {
                    ...
                    (ret = (jsHandler.startActivity(pkgname, className, null, null, -1, null, paramArrayString)));
                }
            } else {
                if (jsHandler.launchLocalAppByClassName) {
                    ret = (jsHandler.launchLocalAppByClassName(pkgname, className,
                        null, null, false));
                }
            }
            if (callback) {
                callback.call(null, ret, "launchLocalView");
            }
        } else if (ua.ios && iosBridge) {
    ...
    env.downloadApp = function(download_info, callback) {
        ...
        if (ua.android && jsHandler) {
            if (jsHandler.downloadApp) { jsHandler.downloadApp(app_id, params); }
        } else if (ua.ios && iosBridge) {
    ...
    env.selectNumber = function(params, callback) {
        ...
        if (ua.android && jsHandler) {
            ...
            jsonParams.slides_array = params.slides_array || ["contact", "calllog", "voip"];
            (jsHandler.goContact) &&
    ...
    env.shareImageToWechat = env.shareWechatByImageUrl;


    /**
         * @description   微信图片分享
         * 以base64编码方式调用微信图片分享功能<br/>
         * [调用示例]
         <pre>
         var shareContent = {
                approach: "wechat/timeline",
                content: {
                    base64ImageString: image_base64,
                    title: "我用触宝，天天抢红包，还不快来一起抢！",
                    content: "5 亿用户信赖的免费电话，从此电话免费打，还有红包抢！"
                }
            }
         _dialerEnv.shareImageBase64(shareContent);
         </pre>
         [源码]
         **/
    env.shareImageBase64 = function(shareContent, callback) {
        var LIMIT_VERSION = 44;
        if (callback) {
            registerAsWindowFunction.call(CallbackFunction, "shareSucceed", function(share_from, org_approach, status) {
        ...
        var _shareContent = typeof shareContent === "string" ? shareContent : JSON.stringify(shareContent);
        if (ua.android && jsHandler) {
            if (BaseParams.apiLevel >= LIMIT_VERSION) {
                (jsHandler.shareMultiple) && (jsHandler.shareMultiple(_shareContent));
            } else {
                (callback) && (callback.call(window, "", "", "failed", ""));
            }
        } else if (ua.ios && iosBridge) {
            iosBridge.callHandler("shareMultiple", _shareContent);
        }
    };


    /**
     * @description 拨打免费电话<br/>
     * [调用示例]
     <pre>
     _dialerEnv.doCall("13000000000", "cootek", true, function(){
        var arg = JSON.stringify(arguments);
        alert(arg);
    });
     </pre>
     ...
     * @param {String} shopName 商户名称,默认为cootek
     * @param {Boolean} popUp 是否展示拨号界面
     * @param {Function} callback  回调方法，回调调用结果
     * @return {Void}
     **/
    env.doCall = function(phone, shopName, popUp, callback) {
        var PHONE_TEST = /^(0|86|17951|\+86)?(13[0-9]|15[012356789]|18[0-9]|14[57]|17[0-9])[0-9]{8}$/;
        if (!PHONE_TEST.test(phone)) {
            (callback) && (callback instanceof Function) &&
            (callback.call(window, { result: "failed", msg: "phone err" }));
            return;
        }
        var shopName = shopName || "cootek";
        if (ua.android && jsHandler) {
            if (jsHandler.callPhoneLevel4) {
                jsHandler.callPhoneLevel4(phone, shopName, popUp);
                (callback) && (callback instanceof Function) &&
                (callback.call(window, { result: "succeed", msg: "call sended" }));
            } else {
                (callback) && (callback instanceof Function) &&
                (callback.call(window, { result: "failed", msg: "not support" }));
            }
        } else if (ua.ios && iosBridge) {
            // not support iOS
            (callback) && (callback instanceof Function) &&
     ...
     */
    env.cookies.getCookie = function(NameOfCookie, default_value) {
